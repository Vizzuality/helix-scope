...
const insertCommon = () => Promise.all([
  'scenarios',
  'measurements',
  'categories'
...
const insertIndicators = () => {
  ...
  const values = indicators.map((i) =>
    `('${i.name}', '${i.slug}', '${i.name_long}', '${i.unit}', '${i.label}', '[${i.colorscheme.map((c) => `"${c}"`)}]', ${i.buckets})`);

  const statement =
    `INSERT INTO meta_indicators(name, slug, name_long, unit, label, colorscheme, buckets) VALUES ${values.join(',')} RETURNING *`;

  return sql(statement, 'indicators');
};

const insertCategoriesIndicators = (type) => {
  const pairs = [];
  const categories_indicators = metadata[`categories_indicators_${type}`];

  Object.keys(categories_indicators).forEach((category) =>
